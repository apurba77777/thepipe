...
def uvfitstobinary(data, scratchdir, ugrids, vgrids, flagparams, figname=None):
	...
	uvwarr		=	np.transpose(np.array([data.par(flagparams['Ukey']),data.par(flagparams['Vkey']),data.par(flagparams['Wkey'])]))
	...
	for ug in range (0,ugrids):
		for vg in range (0,vgrids):
			...
			bindetails	=	[datauvg[3],datauvg[2].par(flagparams['Ukey']),datauvg[2].par(flagparams['Vkey']),\
				 				datauvg[2].par(flagparams['Wkey']),datauvg[2].par('BASELINE')]
...
def uvfitsfrombinary (data, scratchdir, ugrids, vgrids, flagparams):
	...
	for ug in range (0,ugrids):
		for vg in range (0,vgrids):
			...
			if ((len(temparr)%len(recids)) or (len(temparr)%flagparams['Npols']) or (len(temparr)%3)):
			...
			for p in range (0,flagparams['Npols']):
				for d in range (0,3):						
					...
					for l in range (0,len(recids)):
						#print(data[recids[l]].par(ukey),data[recids[l]].par(vkey),data[recids[l]].par(wkey),
						# bindetails[l,1],bindetails[l,2],bindetails[l,3])
...
def baselinestobinary (ANTS, data, scratchdir, breaktimesec, flagparams):
	...
	for scan in range (0,totscans):
		...
		for bline in range (0,nbase):
			...
			bindetails	=	[bindx,bdata.par(flagparams['Ukey']),bdata.par(flagparams['Vkey']),bdata.par(flagparams['Wkey']),\
				 				bdata.par('BASELINE'),scan*np.ones(bindx.shape,dtype=int)]
			bindetails	=	np.array(bindetails)
			bindetails	=	np.transpose(bindetails)
...
def baselinesfrombinary(ANTS, data, scratchdir, flagparams):
...
def flagautocorr(ANTS,data):
	...
	for bline in range (0,len(data.par('BASELINE'))):	
		if (np.isin(data.par('BASELINE')[bline],blid)):
			data[bline].data[:,:,:,:,:,2]	=	-np.abs(data[bline].data[:,:,:,:,:,2])
...
def flagchanlist(ANTS,data,hopelesschans):
	...
	if(nhopeless>0):		
		...
		for hc in hopelesschans:
			if (hc<nchan):
				data.data[:,:,:,:,hc,:,2] = -np.abs(data.data[:,:,:,:,hc,:,2])
